 ...
 * ## The bug
 *
 * `client.db(process.env.MONGODB_DB ?? 'homefarm')`. `??` falls back only on
 ...
 * database is `homefarmdb`. Asserted here because the failure is silent, only
 ...
 * `/homefarmdb` is connected to and then ignored — `MONGODB_DB` is the only
...
describe('the database the server reads', () => {
  it('defaults when nothing says otherwise', () => {
    expect(databaseName({})).toBe('homefarm');
    expect(databaseName({ MONGODB_DB: undefined })).toBe('homefarm');
  });

  it('uses what it is given', () => {
    expect(databaseName({ MONGODB_DB: 'homefarmdb' })).toBe('homefarmdb');
  });
  ...
  it('treats an empty value as absent rather than as a database', () => {
    expect(databaseName({ MONGODB_DB: '' })).toBe('homefarm');
    expect(databaseName({ MONGODB_DB: '   ' })).toBe('homefarm');
  });

  it('does not carry surrounding whitespace into a database name', () => {
    // `MONGODB_DB=homefarmdb ` with a trailing space is a plausible edit, and
   ...
   * them deciding `homefarm` while the other decides `test` is the same class
  ...
  it('agrees with the env schema', () => {
    ...
    for (const value of ['', '   ', 'homefarmdb', ' homefarmdb ']) {
