 ...
 * say so loudly. Set HOMEFARM_REQUIRE_DB=1 — as CI does — to turn an
...
async function connect(uri: string, dbName: string, onStop: () => Promise<void>): Promise<TestDb> {
  ...
  return {
    ...
    stop: async () => {
       ...
       * to do at all: `startTestDb`'s default is `homefarm`, which is the
 ...
 * `homefarm_isolation` until B-3, which is what `vitest.config.ts`'s
...
export async function startTestDb(dbName = DEFAULT_DB_NAME): Promise<TestDb | null> {
  ...
  try {
    ...
    if (process.env.HOMEFARM_REQUIRE_DB === '1') {
      throw new Error(
        `HOMEFARM_REQUIRE_DB=1 but no test database is available.\n${detail}`,
      );
    }
    ...
    console.warn(
      [
        ...
        '',
        '  These suites are the Phase 1 exit gate. CI sets HOMEFARM_REQUIRE_DB=1',
