...
const harness = await startTestDb('homefarm_sign_in');

if (harness) {
  process.env.MONGODB_URI = harness.uri;
  process.env.MONGODB_DB = 'homefarm_sign_in';
...
describe('password hashing', () => {
  it('verifies a correct password', async () => {
    ...
    const { hashPassword, verifyPassword } = await import('@homefarm/api/auth/password');
  ...
  it('treats a malformed stored digest as a wrong password, not a crash', async () => {
    const { verifyPassword } = await import('@homefarm/api/auth/password');
...
describeDb('the credentials provider', () => {
  ...
  async function authorize(credentials: Record<string, unknown>) {
    const { authorizeCredentials } = await import('@homefarm/api/auth/credentials');
