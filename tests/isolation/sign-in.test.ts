...
async function seedOwner(email: string): Promise<{ userId: string; orgId: string }> {
  const { hashPassword } = await import('../../apps/api/src/auth/password');
  const { insertOrg, insertUser } = await import('../../apps/api/src/db/identity');
...
describe('password hashing', () => {
  it('verifies a correct password', async () => {
    ...
    const { hashPassword, verifyPassword } = await import('../../apps/api/src/auth/password');
  ...
  it('treats a malformed stored digest as a wrong password, not a crash', async () => {
    const { verifyPassword } = await import('../../apps/api/src/auth/password');
...
describeDb('identity storage', () => {
  it('round-trips a seeded owner', async () => {
    const { findUserByEmail } = await import('../../apps/api/src/db/identity');
  ...
  it('refuses a second account on the same email', async () => {
    const { applyIndexes } = await import('../../apps/api/src/db/indexes');
...
describeDb('the credentials provider', () => {
  ...
  async function authorize(credentials: Record<string, unknown>) {
    const { authorizeCredentials } = await import('../../apps/api/src/auth/credentials');
