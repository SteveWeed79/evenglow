...
/**
 * The same tenancy assertions, run against BOTH live servers.
 *
 * This is the cheapest available evidence that porting the sync surface to
 * Fastify did not weaken tenancy, and it is worth more than reading the two
 * implementations side by side: they already share `sync/batch.ts` and
 * `sync/snapshot.ts`, so what is actually under test here is the part that is
 * NOT shared — the two auth paths, and the two ways a request becomes a scope.
 *
...
let currentSession: NextSession | null = null;
...
describeDb.each([nextServer, fastifyServer])('$name', (server: Server) => {
  beforeEach(async () => {
