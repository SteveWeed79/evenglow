...
| D3 | **High-volume records are append-only events, not CRUD rows: immutable in value, removable in whole.** | Immutable observations cannot conflict. Sync becomes insert-if-absent. *Amended: this originally claimed a third benefit, "free audit history", and used it to refuse deletes. A removal is `$set archivedAt`, which is repeatable, so it cannot conflict and sync stays insert-if-absent — the two reasons that were load-bearing survive it untouched. The third did not survive being said out loud: a farm that logs twelve eggs against the wrong flock at six in the morning needs to take it back, not to keep a permanent record of having been mistaken. See §4 A10.* |
...
10. **Correction** — every record a person can enter, they can take back, from the screen that shows it to them individually. An append-only record is never *edited* (that is D3, and it is what makes these unable to conflict); it is removed, and the right one logged. The removal is `$set archivedAt`, so it is repeatable, so it cannot conflict either.

    **This is a data-integrity requirement, not a courtesy.** Counts are typed by a person wearing gloves, in the dark, holding a torch, one-handed, at 6am. Mistypes are not an edge case in this application, they are a weekly event, and a system that cannot absorb one turns a slip into a permanent wrong number that every average, forecast and cost-per-egg is then derived from. **The hour meter is the proof**: its readings must climb, so `decideHourReading` refuses anything below the highest recorded — meaning a fat-fingered 9999 on a tractor that has done 999 refuses every true reading for the rest of that machine's life. The rule that was supposed to protect the series was the thing that could destroy it, and removal is the only way out. `highestHours` therefore skips archived readings.
